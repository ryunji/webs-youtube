    ...
    return (
        ...
        <BrowserRouter>
            ...
            <Routes>
                ...
                <Route path='/gsap'                 element={<Gsap/>}/>             {/* 2. 해당 컴포넌트를 불러올 수 있도록 element를 설정하는 것. */}
                <Route path='/port'                 element={<Port/>}/>
                <Route path='/youtube'              element={<Youtube/>}/>
                <Route path='/channel/:channelID'   element={<Channel/>}/>          
                <Route path='/video/:videoID'       element={<Video/>}/>            {/* 3. url에 그냥 search만 쓰면 Not.jsx 페이지를 가져옴. search 페이지를 제대로 호출하기 위해서는 뒤에 키워드(문자열)를 작성해야 한다. ex) /search/개발 */}
                <Route path='/search/:searchID'     element={<Search/>}/>
                <Route path='/*'                    element={<Home/>}/>             {/* 4. 컴포넌트는 단어 앞에 대문자로 표기 */}
                                                                                    {/* 5. 여기에는 명시적으로 연동되어 있지 않지만 없는 페이지를 호출하면 Not.jsx 페이지를 호출 함. */}
            </Routes>
        </BrowserRouter>
    )
}
